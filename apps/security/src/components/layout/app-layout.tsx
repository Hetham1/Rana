...
const pageTitles: Record<string, string> = {
  "/": "میزکار حراست",
  "/requests": "درخواست ورود",
  "/dispatch": "سفارش‌های آماده خروج",
  "/scanner": "کنترل مجوز خروج",
};

export default function AppLayout() {
  const navigate = useNavigate();
  const location = useLocation();
  const fullName = localStorage.getItem("fullName") || "کاربر حراست";
      ...
      <header className="app-header sticky top-3 z-40 mx-3 mt-3 rounded-2xl px-4 py-3 sm:mx-5 sm:px-6">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-3">
          <div className="flex min-w-0 items-center gap-3">
            <span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-gradient-to-br from-slate-900 to-blue-700 text-white shadow-lg shadow-blue-900/20">
              <ShieldCheck className="h-5 w-5" aria-hidden="true" />
            </span>
            <div className="min-w-0">
              <h1 className="truncate text-base font-bold text-slate-900 sm:text-lg">{pageTitles[location.pathname] || "کنترل حراست"}</h1>
              <p className="truncate text-xs text-slate-500">{fullName} · دروازه خروج</p>
            </div>
          </div>
