...
export default function LoginPage({ onLogin }: LoginProps) {
  ...
  async function handleLogin(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!["admin", "security"].includes(session.role)) {
        setError("این حساب اجازه ورود به سامانه حراست را ندارد.");
        return;
      }
    ...
    <main className="relative grid min-h-screen place-items-center overflow-hidden px-4 py-10" dir="rtl">
      ...
      <section className="surface-card page-shell relative w-full max-w-md overflow-hidden p-6 sm:p-9">
        <div className="mb-8 text-center">
          ...
          <h1 className="text-2xl font-bold text-slate-900">ورود حراست</h1>
          <p className="mt-2 text-sm leading-6 text-slate-500">کنترل مجوز خروج و ثبت اطلاعات حمل را سریع و دقیق انجام دهید.</p>
        </div>
